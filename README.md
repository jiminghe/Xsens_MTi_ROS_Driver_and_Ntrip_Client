...
#### Note: the UTC Time, SampleTimeFine, Status Word, Latitude and Longitude, Pvt Data needs to be enabled, in order to get GPGGA data: MT Manager - Device Settings - Output Configuration , select "UTC Time, Sample TimeFine, Status Word, Latitude and Longitude, Pvt Data" and other required data, click "Apply"
...
## Changes made to the MTi ROS Driver:
 ...
 - add ``xsens_time_handler.h`` and ``xsens_time_handler.cpp`` under ``src`` folder, to give user option to use UTC Time from the MTi sensor.
